...
from lib.categories import product_categories_list, product_categories_weights
from lib.coupons import special_events
from lib.payment_methods import payment_methods
...
# Randomize sex weights
def generate_random_sex_weights():
    # Generate random weights for M, F, O
    sex_weights = [
        random.uniform(0.4, 0.5) for _ in range(2)
    ]  # Random weights for M and F
    sex_weights.append(1 - sum(sex_weights))  # Weight for O is the remaining value
    return sex_weights


sex_options = ["M", "F", "O"]
sex_weights = generate_random_sex_weights()
total_sex_weight = sum(sex_weights)
normalized_sex_weights = [w / total_sex_weight for w in sex_weights]
...
def generate_synthetic_data(num_rows=1000, month=None, year=None):
    ...
    for _ in range(num_rows):
        ...
        sex = random.choices(sex_options, weights=normalized_sex_weights, k=1)[0]
